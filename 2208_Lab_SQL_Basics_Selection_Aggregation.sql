...
transaction_type = replace(transaction_type, 'VYDAJ' , 'OUTGOING');

SELECT account_id, transaction_type, CAST(sum(amount) AS UNSIGNED) AS total_amount FROM trans
WHERE account_id = 396
...
SELECT *, CAST(sum(amount) AS UNSIGNED) AS total_amount
FROM trans
WHERE account_id = 396
GROUP BY transaction_type;

# SELECT sum(amount) AS sum_inc FROM trans WHERE transaction_type = "INCOMING"  
# WHERE account_id = 396;
