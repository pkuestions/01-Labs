...
WHERE payment_date BETWEEN 050801 AND 050831               # or "2005-08-01" AND "2005-08-31"
GROUP BY staff_id
ORDER BY first_name;

SELECT concat(first_name, " ", last_name) AS actor, sum(amount) AS total_amount
FROM payment 
JOIN staff USING (staff_id)
...
SELECT title, count(DISTINCT actor_id) AS actor_count
FROM film_actor
...
SELECT title, count(rental_id) AS amount_rented
FROM rental
